...
const DATA_DISCLAIMER =
  "Motormila aggregates public Sri Lanka vehicle marketplace data. Treat outputs as decision support, not a binding valuation or offer.";
...
function drawFooter(doc: JsPDFDoc, report: ProReportPayload, pageWidth: number, pageHeight: number, pageNumber: number) {
  ...
  doc.text(`Motormila Pro Intelligence | ${new Date(report.generatedAt).toLocaleString("en-LK")}`, 40, pageHeight - 24);
  ...
  popup.document.write(`
    ...
    <html>
      ...
      <body>
        <div class="page">
          <header class="hero">
            <div class="kicker">Motormila Pro Intelligence</div>
