...
function dealBandChip(score: number): string {
  ...
  return "border-amber-400/15 bg-amber-400/5 text-amber-300/80";
}

function dealMeter(score: number): string {
  return dealBand(score) === "watch" ? "bg-amber-500/60" : "bg-emerald-500/60";
}
    ...
    <div className="min-h-screen">
      <section className="border-b border-white/[0.04]">
        <div className="mx-auto max-w-[1320px] px-5 py-10 sm:px-6 sm:py-12">
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-600">Best picks</p>
          <h1 className="mt-3 font-display text-[1.75rem] font-semibold tracking-tight text-foreground sm:text-[2.25rem]">Deal-score picks.</h1>
      ...
      <div className="mx-auto max-w-[1320px] px-5 py-8 sm:px-6 lg:py-10 space-y-8">
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-4 rounded-xl border border-dashed border-white/[0.05] py-16 text-center">
            <SearchX className="h-5 w-5 text-zinc-500" />
            <p className="text-[13px] text-zinc-400">{error}</p>
            <Link to="/#market" className="rounded-lg border border-white/[0.06] px-4 py-2 text-[11px] font-semibold text-zinc-300 no-underline hover:bg-white/[0.03]">Open inventory</Link>
          </div>
        ) : picks.length === 0 ? (
          <div className="flex flex-col items-center gap-4 rounded-xl border border-dashed border-white/[0.05] py-16 text-center">
            <TrendingUp className="h-5 w-5 text-zinc-500" />
            <p className="text-[13px] text-zinc-400">No vehicles meet the deal-score gate right now.</p>
            <Link to="/#market" className="rounded-lg border border-white/[0.06] px-4 py-2 text-[11px] font-semibold text-zinc-300 no-underline hover:bg-white/[0.03]">Browse inventory</Link>
          </div>
        ) : (
          <>
            {/* Featured */}
            {featured && (() => {
              ...
              return (
                <article className="grid overflow-hidden rounded-xl border border-white/[0.05] bg-[hsl(220,8%,6%)] lg:grid-cols-[1.1fr_1fr]">
                  ...
                  <div className="flex flex-col justify-between gap-5 p-6">
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-semibold text-zinc-500">{featured.source}</span>
                      ...
                      <Link to={`/listing/${featured.id}`} className="block no-underline">
                        ...
                        <p className="mt-1 text-[12px] text-zinc-500">{featured.year || "N/A"} · {featured.district || "LK"}</p>
                      </Link>
                      ...
                      <div className="h-1 overflow-hidden rounded-full bg-zinc-800/50">
                        <div className={`h-full rounded-full ${dealMeter(score)}`} style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <Link to={`/listing/${featured.id}`} className="flex h-10 flex-1 items-center justify-center rounded-lg bg-[var(--gold)] text-[10px] font-bold uppercase tracking-[0.08em] text-black no-underline hover:bg-[var(--gold-bright)]">Open detail</Link>
                      {featured.external_url && (
                        <a href={featured.external_url} target="_blank" rel="noopener noreferrer" className="flex h-10 items-center gap-1 rounded-lg border border-white/[0.06] px-3 text-[10px] font-semibold text-zinc-400 no-underline hover:text-zinc-200">
            ...
            {rest.length > 0 && (
              <div>
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="text-[10px] font-semibold uppercase tracking-[0.14em] text-zinc-500">Ranked picks</h3>
                  <span className="text-[10px] text-zinc-600 num">{rest.length} more</span>
                </div>
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {rest.map((listing, idx) => {
                    ...
                    const pct = topScore > 0 ? Math.min(100, Math.round((score / topScore) * 100)) : 0;
                    return (
                      <article key={listing.id} className="group flex flex-col overflow-hidden rounded-xl border border-white/[0.04] bg-[hsl(220,8%,5.5%)] transition-colors hover:border-white/[0.08]">
                        <Link to={`/listing/${listing.id}`} className="relative block aspect-[16/10] overflow-hidden bg-black/30 no-underline">
                          ...
                          <span className="absolute left-2 top-2 rounded-md border border-white/[0.1] bg-black/50 px-1.5 py-0.5 text-[10px] font-bold text-white num backdrop-blur-sm">{String(idx + 2).padStart(2, "0")}</span>
                        </Link>
                        <div className="flex flex-1 flex-col gap-3 p-4">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] text-zinc-600">{listing.source}</span>
                          ...
                          <Link to={`/listing/${listing.id}`} className="block no-underline">
                            ...
                            <p className="mt-0.5 text-[10px] text-zinc-500">{listing.year || "N/A"} · {listing.district || "LK"}</p>
                          </Link>
                          ...
                          <div className="mt-auto h-1 overflow-hidden rounded-full bg-zinc-800/50">
                            <div className={`h-full rounded-full ${dealMeter(score)}`} style={{ width: `${pct}%` }} />
                          </div>
                        </div>
                      </article>
