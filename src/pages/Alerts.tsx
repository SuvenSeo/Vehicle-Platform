      ...
      {matchData && matchData.results.length > 0 && (
        <div className="space-y-6">
          {matchData.results.map((result) => (
            <div key={result.alert_id} className="rounded-xl border border-border bg-card overflow-hidden">
              <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
                ...
                <Badge variant="secondary" className="shrink-0 text-[11px] font-bold num">
                    ...
                    <div key={listing.id} className="flex items-center justify-between gap-3 px-4 py-3">
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        ...
                        <Link
                          ...
                          className="flex h-7 w-7 items-center justify-center rounded-md border border-border text-muted-foreground no-underline transition-colors hover:text-foreground"
                        >
...
function CreateAlertForm({ onCreated, onCreate }: CreateAlertFormProps) {
  ...
  return (
    ...
    >
      <p className="text-[11px] font-bold uppercase tracking-[0.1em] text-muted-foreground">New alert</p>
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label htmlFor="alert-make" className="mb-1 block text-[11px] font-medium text-muted-foreground">Make</label>
          <Input id="alert-make" value={make} onChange={(e) => setMake(e.target.value)} placeholder="Toyota" className="h-9 text-sm" />
        </div>
        <div>
          <label htmlFor="alert-model" className="mb-1 block text-[11px] font-medium text-muted-foreground">Model</label>
          <Input id="alert-model" value={model} onChange={(e) => setModel(e.target.value)} placeholder="Aqua" className="h-9 text-sm" />
        </div>
        <div>
          <label htmlFor="alert-district" className="mb-1 block text-[11px] font-medium text-muted-foreground">District</label>
          <Input id="alert-district" value={district} onChange={(e) => setDistrict(e.target.value)} placeholder="Colombo" className="h-9 text-sm" />
        </div>
        <div>
          <label htmlFor="alert-price" className="mb-1 block text-[11px] font-medium text-muted-foreground">Max price (LKR)</label>
          <Input
            id="alert-price"
            value={maxPrice}
      ...
      {formError && (
        <p role="alert" className="text-[11px] text-destructive/80">{formError}</p>
      )}
      <div className="flex gap-2">
        <Button type="submit" size="sm" disabled={saving} className="h-9 flex-1 text-[12px] font-semibold">
          {saving ? "Saving…" : "Save alert"}
        </Button>
        <Button type="button" variant="ghost" size="sm" onClick={() => { setOpen(false); setFormError(null); }} className="h-9 px-3 text-[12px]">
          Cancel
        </Button>
      </div>
    </form>
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        <Link
          ...
          className="flex h-7 items-center gap-1 rounded-md border border-primary/15 bg-primary/5 px-2.5 text-[10px] font-semibold text-primary/80 no-underline transition-colors hover:bg-primary/10"
        >
          Browse
        </Link>
        <button
          ...
          className="flex h-7 w-7 items-center justify-center rounded-md border border-border text-muted-foreground transition-colors hover:border-destructive/30 hover:text-destructive disabled:opacity-40"
        >
    ...
    <div className="mx-auto max-w-2xl px-5 py-10 sm:px-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          <Bell className="h-5 w-5 text-primary/70" aria-hidden />
          <h1 className="text-2xl font-semibold tracking-tight text-foreground">Market Alerts</h1>
        </div>
      ...
      {/* Active alerts */}
      <section aria-labelledby="active-alerts-heading" className="mb-10">
        <div className="mb-4 flex items-center justify-between">
          <h2 id="active-alerts-heading" className="text-base font-semibold tracking-tight text-foreground">
            Active alerts
          </h2>
          {!loading && (
        ...
        {loading && (
          <div className="space-y-2" aria-label="Loading alerts">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-16 animate-pulse rounded-xl border border-border bg-foreground/[0.02]" />
            ))}
          </div>
        )}

        {!loading && error && alerts.length === 0 && (
          <div className="rounded-lg border border-destructive/20 bg-destructive/5 px-4 py-3 text-[12px] text-destructive/80" role="alert">
            {error}
          </div>
        )}
        ...
        {!loading && !error && alerts.length === 0 && (
          <div className="rounded-xl border border-dashed border-border py-10 text-center">
            <BellOff className="mx-auto mb-3 h-6 w-6 text-muted-foreground/40" aria-hidden />
            <p className="text-[12px] text-muted-foreground">No active alerts. Create one above to get started.</p>
          </div>
        )}
        ...
        {!loading && alerts.length > 0 && (
          <div className="space-y-2">
            {alerts.map((alert) => (
              <AlertRow key={alert.id} alert={alert} onDelete={remove} />
            ))}
          </div>
        )}
              ...
              {localAlerts.map((a) => (
                <div key={a.id} className="flex items-center justify-between gap-3 rounded-lg border border-border bg-surface p-3">
                  <div className="min-w-0">
                    ...
                    {a.target_price_lkr ? (
                      <p className="mt-0.5 text-[11px] text-muted-foreground">Under {formatPrice(a.target_price_lkr)}</p>
                    ) : null}
                  </div>
                  <Link
